...
class _SettingsPageState extends ConsumerState<SettingsPage> {
  ...
  Future<void> _showEndpointDialog({EndpointConfig? endpoint}) async {
    final isEdit = endpoint != null;
    final urlController = TextEditingController(text: endpoint?.url ?? '');
    final apiKeyController = TextEditingController(text: endpoint?.apiKey ?? '');
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                const Text('Endpoint 管理', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)),
                FilledButton.icon(
                  onPressed: () => _showEndpointDialog(),
                  icon: const Icon(Icons.add, size: 18),
                  label: const Text('新增 Endpoint'),
                ),
              ],
            ),
            const SizedBox(height: 12),
    ...
    return endpointsAsync.when(
      data: (endpoints) {
        if (endpoints.isEmpty) {
          return const Center(
            child: Text('暂无 Endpoint，点击右上角"新增 Endpoint"添加',
            ...
            return Card(
              ...
              child: ListTile(
                leading: CircleAvatar(
                  ...
                  child: Text('#${ep.id}', style: TextStyle(fontSize: 12, color: Colors.blue.shade700)),
                ),
                ...
                trailing: Row(
                  ...
                  children: [
                    IconButton(
                      ...
                      tooltip: '编辑',
                      onPressed: () => _showEndpointDialog(endpoint: ep),
                    ),
                    IconButton(
